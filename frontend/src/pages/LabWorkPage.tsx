...
const labWorkColumns: ColumnDef<LabWorkDTO>[] = [
    ...
    { accessorKey: 'coordinatesId', header: 'Coordinates ID' },
...
type ActiveFilter = { by: keyof LabWorkDTO; value: string } | null;
...
export const LabWorkPage = () => {
    ...
    const applyFilter = (data: LabWorkDTO[], f: ActiveFilter) => {
        if (!f) return data;
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            setOriginalData(labWorks); // filteredData пересчитается ниже из activeFilter
        } catch (err) {
    ...
    const handleFilterSubmit = (filterBy: string, filterValue: string) => {
        const by = filterBy as keyof LabWorkDTO;
    ...
    return (
        <div className={styles.page}>
            ...
            <Modal isOpen={activeModal === 'filter'} onClose={closeModal} title="Filter">
                <FilterForm onClose={closeModal} columns={labWorkColumns} onSubmit={handleFilterSubmit} />
            </Modal>

            <Modal isOpen={activeModal === 'create'} onClose={closeModal} title="Create LabWork">
                <LabWorkForm onSuccess={handleSuccess} />
            </Modal>

            <Modal
                isOpen={activeModal === 'read'}
                onClose={closeModal}
                title={`Details for LabWork #${detailedItem?.id || ''}`}
            >
                {detailedItem ? <LabWorkDetails labWork={detailedItem} /> : <p>Loading details...</p>}
            </Modal>

            <Modal
                isOpen={activeModal === 'update'}
                onClose={closeModal}
                title={`Update LabWork #${selectedRow?.id}`}
            >
                <LabWorkForm onSuccess={handleSuccess} initialData={selectedRow} />
            </Modal>
